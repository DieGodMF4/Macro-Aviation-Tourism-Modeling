...
> **[INSERTAR FIGURA 1.1 AQUÍ]**
> *Descripción para TFG:* Gráfico de barras horizontales mostrando la contribución del sector turístico al PIB por país europeo. Destacar visualmente a Polonia (aprox. 4 %) frente a la media europea (10 %) y economías altamente dependientes como Croacia o España. Sirve para evidenciar el margen de crecimiento y la importancia estratégica de predecir la demanda para fomentar este sector. Fuente sugerida: Datos del WTTC 2025.
...
> **[INSERTAR FIGURA 2.1 AQUÍ]**
> *Descripción para TFG:* Diagrama conceptual (tipo flowchart o esquema matemático gráfico) ilustrando cómo se construye la variable $P_{it}$. Ayuda al tribunal a asimilar la interacción entre la inflación relativa ($CPI$) y los tipos de cambio ($EX$) sin detenerse estrictamente en el álgebra.
...
> **[INSERTAR FIGURA 2.2 AQUÍ]**
> *Descripción para TFG:* Gráfico de línea de las pernoctaciones anuales totales en Polonia (2004–2025), destacando con una franja sombreada (o *span* vertical) el impacto del COVID-19.

Este volumen turístico ha sido posible gracias a una profunda modernización de la infraestructura aeroportuaria y a la penetración de las aerolíneas de bajo coste (Skift, 2023; Huderek-Glapska y Nowak, 2016).

> **[INSERTAR FIGURA 2.3 AQUÍ]**
> *Descripción para TFG:* Gráfico de área apilada (*stacked area chart*) mostrando la evolución de la cuota de mercado de aerolíneas LCC frente a aerolíneas tradicionales en Polonia (aprox. 5 % antes de 2004 frente a >59 % en 2023).
...
> **[INSERTAR FIGURA 2.4 - OPCIONAL AQUÍ]**
> *Descripción para TFG:* Línea temporal (timeline) resumiendo la evolución algorítmica: 1990s (ARIMA) $\rightarrow$ 2000s (Econometría VAR) $\rightarrow$ 2010s (Machine Learning) $\rightarrow$ 2020s (Deep Learning y Bayesiano).
...
6. Generar simulaciones de predicción bajo distintos escenarios macroeconómicos y de conectividad aérea (tendencial, optimista y pesimista).
...
> **[INSERTAR FIGURA 3.1 AQUÍ]**
> *Descripción para TFG:* Gráfico de radar (*spider chart*) visualizando las puntuaciones de la siguiente tabla, destacando los ODS 8 y 9.
...
6. **Despliegue:** Documentación sistemática de los resultados en la memoria final y publicación del repositorio de código.

## 4.2. Fases de desarrollo y Análisis Exploratorio

> **[INSERTAR FIGURA 4.2 AQUÍ]**
...
El análisis descriptivo univariante revela la naturaleza estructural de la variable dependiente (Pernoctaciones internacionales). La serie exhibe una tendencia alcista constante hasta 2019, interrumpida abruptamente por un colapso del 96,3 % en abril de 2020 debido a las restricciones sanitarias. 

> **[INSERTAR FIGURA 4.3 AQUÍ]**
> *Descripción para TFG:* Gráfico multipanel del *Notebook 01*. (Panel 1: Serie temporal bruta; Panel 2: Crecimiento interanual (YoY); Panel 3: Suma móvil de 12 meses).
> *Sugerencia de implementación en Python (Matplotlib):*
```python
fig, (ax1, ax2, ax3) = plt.subplots(3, 1, figsize=(12, 10), sharex=True)
ax1.plot(df.index, df['nights'], color='blue')
ax1.set_title('Pernoctaciones Mensuales')
ax2.plot(df.index, df['nights_yoy_growth'], color='orange')
ax2.axhline(0, color='black', linestyle='--')
ax2.set_title('Crecimiento Interanual (YoY %)')
ax3.plot(df.index, df['nights'].rolling(12).sum(), color='green')
ax3.set_title('Suma Móvil 12 Meses (Tendencia Anualizada)')
plt.tight_layout()
plt.show()
```
...
> *Sugerencia de implementación en Python (Seaborn):*
```python
pivot_df = df.pivot_table(values='nights', index=df.index.year, columns=df.index.month)
plt.figure(figsize=(10, 6))
sns.heatmap(pivot_df, cmap='YlGnBu', annot=False)
plt.title('Mapa de Calor: Pernoctaciones por Año y Mes')
plt.xlabel('Mes')
plt.ylabel('Año')
plt.show()
```
